...
getROC <- function(train, clin.train, test, clin.test){
  ...
  balanced.auc <- max((b.auc_ROCR@x.values[[1]] + b.auc_ROCR@y.values[[1]]) / 2)
  print(balanced.auc)
  
  df <- data.frame(FalsePositive=c(perf@x.values[[1]]),
                   TruePositive=c(perf@y.values[[1]]))
  out <- list(balanced.auc, df)
...
Model_Comparison_Plot <- ggplot(Results, aes(x = forcats::fct_relevel(Model, mean_order), y = AUROC, color = Model)) +
  ...
  annotate(geom = "text", x = 3, y = 0.25, label = "Balanced AUROC", color = 'black', size = 5) +
...
ggsave(Model_Comparison_Plot, file = "../figures/fig5_redo.png",  width = 7, height = 5, dpi = 600)
```