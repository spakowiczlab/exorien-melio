...
```{r}
simp_auc <- kTest(response, simpson, seed_list)
simp_auc$signature <- "Simpson's diversity"

shan_auc <- kTest(response, shannon, seed_list)
shan_auc$signature <- "Shannon's diversity"
```

prep for boxplot
```{r}
aucs <- rbind(simp_auc, shan_auc)
```
