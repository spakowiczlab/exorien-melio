...
# Read in counts
counts <- read.delim("/fs/ess/PAS1695/projects/exorien/data/k2bout.txt") %>%
...
ra$total <- ave(ra$counts, ra$sample, FUN=sum)

ra <- ra %>%
  mutate(exo.ra = counts/total)
```

prep counts for calc
```{r}
counts <- counts %>%
  remove_rownames %>% 
...
<!-- ```{r} -->
<!-- taxawide <- taxa %>% -->
<!--   mutate(exo.ra = as.integer(exo.ra)) %>% -->
...
<!-- diversity calcs w/ counts - Shannon entropy, Simpson’s index, Chao1 -->
<!-- ```{r} -->
<!-- # simpson  -->
<!-- test <- as.data.frame(diversity(counts, index = "simpson")) -->
<!-- test$sample <- row.names(test) -->
<!-- rownames(test) <- NULL -->
<!-- simpson <- test %>% -->
<!--   dplyr::rename("alpha_diversity" = `diversity(counts, index = "simpson")`) -->

<!-- # shannon -->
<!-- test <- as.data.frame(diversity(counts, index = "shannon")) -->
<!-- test$sample <- row.names(test) -->
<!-- rownames(test) <- NULL -->
<!-- shannon <- test %>% -->
<!--   dplyr::rename("alpha_diversity" = `diversity(counts, index = "shannon")`) -->
<!-- ``` -->
...
prep for boxplot
```{r}
aucs <- rbind(simp_auc, shan_auc, invsim_auc, chao_auc, ace_auc)
```
...
ggsave(plot = plot, file = "../figures/diversityAUCboxplots.png", dev = "png", width = 6, height = 5, dpi = 400)
```
...
ggsave(file = "../figures/diversityBoxplots.png", dev = "png", width = 6, height = 5, dpi = 400)
```
