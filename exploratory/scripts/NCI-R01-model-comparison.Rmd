...
```{r}
grant.ver <- Results %>%
  filter(Model %in% c('Microbe', 'Imm_Act_Z_Score', 'Imm_Act_Z_Microbe'))

Model_Comparison_Plot <- ggplot(grant.ver , aes(x = forcats::fct_relevel(Model, mean_order), y = AUROC, color = Model)) +
  ...
  scale_color_manual(values = c("#332288", "#999933", "chocolate"))
...
ggsave(Model_Comparison_Plot, file = "../figures/grantVersion_AUCboxplots.svg", dev = "svg", width = 3, height = 3, dpi = 600)
```